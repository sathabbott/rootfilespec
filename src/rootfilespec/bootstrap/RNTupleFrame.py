...
class RNTupleRecordFrame_SchemaExtension(ROOTSerializable):
    ...
    def read(cls, buffer: ReadBuffer):
        ...
        unknown, buffer = buffer.consume(fheader.fSize - 8 - buffer.relpos + payload_start_pos)
        ...
        print(f"\033[1;32m\t\tDone reading RNTuple Schema Extension Record Frame! {fheader}\033[0m")
...
class RNTupleListFrame_ClusterGroups(ROOTSerializable):
    ...
    def read(cls, buffer: ReadBuffer):
        ...
        for _ in range(fheader.nItems):
            clusterGroupRecordFrame, buffer = RNTupleRecordFrame_ClusterGroup.read(buffer)
            # Verify that we read a record frame
            if clusterGroupRecordFrame.fheader.fType != 0:
                raise ValueError(f"Expected a (cluster group) record frame, but got a frame of type {clusterGroupRecordFrame.fheader.fType=}")
            clusterGroupRecordFrames.append(clusterGroupRecordFrame)

        #### Consume any unknown trailing information in the frame
        unknown, buffer = buffer.consume(fheader.fSize - 12 - buffer.relpos + payload_start_pos)
        # Unknown Bytes = Payload Size - Payload Bytes Read
        #   Payload Size        = frame length  - 12 bytes for the header
        #   Payload Bytes Read  = buffer.relpos - payload_start_pos


        print(f"\033[1;32m\t\tDone reading RNTuple List Frame of Cluster Group Record Frames!\033[0m {buffer.info()}")
...
class RNTupleRecordFrame_ClusterGroup(ROOTSerializable):
    ...
    def read(cls, buffer: ReadBuffer):
        ...
        """
        print(f"\033[1;36m\t\t\tReading RNTuple Cluster Group Record Frame;\033[0m {buffer.info()}")
        ...
        print(f"\033[1;32m\t\t\tDone reading RNTuple Cluster Group Record Frame!\033[0m {buffer.info()}")
        return cls(fheader, minEntryNumber, entrySpan, nClusters, pagelistLink, unknown), buffer
...
class RNTupleListFrame_ClusterSummaries(ROOTSerializable):
    ...
    def read(cls, buffer: ReadBuffer):
        ...
        for _ in range(fheader.nItems):
            clusterSummaryRecordFrame, buffer = RNTupleRecordFrame_ClusterSummary.read(buffer)
            # Verify that we read a record frame
            if clusterSummaryRecordFrame.fheader.fType != 0:
                raise ValueError(f"Expected a (cluster summary) record frame, but got a frame of type {clusterSummaryRecordFrame.fheader.fType=}")
        ...
        print(f"\033[1;32m\t\tDone reading RNTuple List Frame of Cluster Summary Record Frames!\033[0m {buffer.info()}")
...
class RNTupleRecordFrame_ClusterSummary(ROOTSerializable):
    ...
    def read(cls, buffer: ReadBuffer):
        ...
        """
        print(f"\033[1;36m\t\t\tReading RNTuple Cluster Summary Record Frame;\033[0m {buffer.info()}")
        ...
        print(f"\033[1;32m\t\t\tDone reading RNTuple Cluster Summary Record Frame!\033[0m")
...
class RNTupleListFrame_PageLocations_Clusters(ROOTSerializable):
    ...
    def read(cls, buffer: ReadBuffer):
        ...
        columnListFrames = [] 
        for _ in range(fheader.nItems):
            columnListFrame, buffer = RNTupleListFrame_PageLocations_Columns.read(buffer)
            # Verify that we read a list frame
            if columnListFrame.fheader.fType != 1:
                raise ValueError(f"Expected a (page locations: column) list frame, but got a frame of type {columnListFrame.fheader.fType=}")
        ...
        print(f"\033[1;32m\t\tDone reading RNTuple Page Locations Cluster (Top-Most) List Frame!\033[0m {buffer.info()}")
        return cls(fheader, columnListFrames, unknown), buffer
    
    def read_list(self, fetch_data: DataFetcher) -> list[RNTupleListFrame_PageLocations_Columns]:
        return [columnListFrame.read_list(fetch_data) for columnListFrame in self.columnListFrames]
...
class RNTupleListFrame_PageLocations_Columns(ROOTSerializable):
    ...
    def read(cls, buffer: ReadBuffer):
        ...
        """
        print(f"\033[1;36m\t\t\tReading RNTuple Page Locations Column (Outer) List Frame;\033[0m {buffer.info()}")
        ...
        for _ in range(fheader.nItems):
            ...
            if pageListFrame.fheader.fType != 1:
                raise ValueError(f"Expected a (page locations: page) list frame, but got a frame of type {pageListFrame.fheader.fType=}")
        ...
        print(f"\033[1;32m\t\t\tDone reading RNTuple Page Locations Column (Outer) List Frame!\033[0m {buffer.info()}")
        return cls(fheader, pageListFrames, unknown), buffer
    
    def read_list(self, fetch_data: DataFetcher) -> list[RNTupleListFrame_PageLocations_Pages]:
        return [pageListFrame.read_list(fetch_data) for pageListFrame in self.pageListFrames]
...
class RNTupleListFrame_PageLocations_Pages(ROOTSerializable):
    ...
    def read(cls, buffer: ReadBuffer):
        ...
        print(f"\033[1;36m\t\t\t\tReading RNTuple Page Locations Page (Inner) List Frame;\033[0m {buffer.info()}")
        ...
        print(f"\033[1;32m\t\t\t\tDone reading RNTuple Page Locations Page (Inner) List Frame!\033[0m {buffer.info()}")
        return cls(fheader, pageLocations, elementOffset, isSuppressed, compressionSettings, unknown), buffer

    def read_list(self, fetch_data: DataFetcher) -> list[RNTuple_PageLocation]:
        return [pageLocation.read_page(fetch_data) for pageLocation in self.pageLocations]
...
class RNTuple_PageLocation(ROOTSerializable):
    ...
    def read_page(self, fetch_data: DataFetcher) -> ROOTSerializable:
        ...
        if compressed:
            raise NotImplementedError("Compressed pages are not yet supported")
