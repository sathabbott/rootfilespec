...
class RNTupleEnvelope(ROOTSerializable):
    ...
    def read(cls, buffer: ReadBuffer):
        ...
        print(
            f"\033[1;32mDone reading RNTuple Envelope! {typeID=} ({ENVELOPE_TYPE_MAP[typeID]}), {length=}\033[0m"
        )
    ...
    def print_info(self):
        ...
        for attr_name, attr_value in vars(self).items():
            ...
            # Print payload information
                ...
                print(
                    f"\033[1;35m\t------------------------ {self.get_type()} Envelope Payload ------------------------\033[0m"
                )
                self._print_payload(self.payload, depth=1)
                print(
                    f"\033[1;35m\t---------------------- End {self.get_type()} Envelope Payload ----------------------\033[0m"
                )

        print(
            f"\033[1;35m------------------------------ End RNTuple {self.get_type()} Envelope Info ------------------------------\n\033[0m"
        )
    ...
    def _print_payload(self, payload: Any, depth: int) -> None:
        ...
        for attr_name, attr_value in vars(payload).items():
            if isinstance(attr_value, list):  # Check if the attribute is a list
                ...
                print(
                    f"\033[1;36m{tabs}{attr_name} (type: list[{type(attr_value[0]).__name__}]):\033[0m"
                )
                ...
                "Frame" in attr_name
            ):  # Check if the attribute has nested attributes
                # print(f"\033[1;33m\ttrigger dict\033[0m")
                print(
                    f"\033[1;34m{tabs}{attr_name} (type: {type(attr_value).__name__}):\033[0m"
                )
...
class RNTupleFooterEnvelope_payload(ROOTSerializable):
    ...
    def read(cls, buffer: ReadBuffer):
        ...
        print(
            f"\033[1;36m\tReading RNTuple Footer Envelope Payload;\033[0m {buffer.info()}"
        )
    ...
    def get_pagelist(self, fetch_data: DataFetcher) -> list[RNTupleEnvelope]:
        ...
        print("\033[1;36mReading RNTuple Page Lists\033[0m")
        ...
        for i, clusterGroupListFrame in enumerate(
            self.clusterGroupListFrame.clusterGroupRecordFrames
        ):
            ...
            print(
                f"\033[1;32m---- Done reading RNTuple Page List Envelope {i}! ----\033[0m\n"
            )

        print("\033[1;32mDone reading RNTuple Page Lists!\033[0m\n")
...
class RNTuplePageListEnvelope_payload(ROOTSerializable):
    ...
    def read(cls, buffer: ReadBuffer):
        ...
        print(
            f"\033[1;36m\tReading RNTuple Page List Envelope Payload;\033[0m {buffer.info()}"
        )
    ...
    def get_pages(self, fetch_data: DataFetcher) -> list[list[list[RNTuplePageInfo]]]:
        ...
        print("\033[1;36mReading RNTuple Pages\033[0m")

        #### Get the Pages
        pages = self.pageLocationsNestedListFrame.read_list(fetch_data)

        print("\033[1;32mDone reading RNTuple Pages!\033[0m\n")
        return pages
